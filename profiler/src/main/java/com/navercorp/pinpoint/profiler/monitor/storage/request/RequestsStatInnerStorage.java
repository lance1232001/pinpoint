...
public class RequestsStatInnerStorage {
    ...
    private final int needsFlushDataSize;

    private int currentDataSize = 0;

    public RequestsStatInnerStorage(int needsFlushDataSize) {
        this.needsFlushDataSize = needsFlushDataSize;
    }

    public boolean storeAndisNeedFlush(RequestsStatInfo requestsStatInfo) {
        ...
        if (currentDataSize > needsFlushDataSize) {
            return true;
        } else {
            return false;
        }
