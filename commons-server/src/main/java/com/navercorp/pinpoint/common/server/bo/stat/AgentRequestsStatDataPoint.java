...
public class AgentRequestsStatDataPoint {
    ...
    public Collection<RequestsStatSummaryBo> getAgentRequestsStatSummaryDataList(String agentId, long startTimestamp) {
        ...
        Set<Map.Entry<Integer, Collection<StartAndElapsedTime>>> requestsSet = statusMap.entrySet();
        for (Map.Entry<Integer, Collection<StartAndElapsedTime>> statusRequestsEntry : requestsSet) {
            Collection<StartAndElapsedTime> timeCollection = statusRequestsEntry.getValue();
            if (CollectionUtils.isEmpty(timeCollection)) {
                continue;
            }

            Integer status = statusRequestsEntry.getKey();
            RequestsStatSummaryBo agentRequestsStatSummaryData = createSummaryData(status, timeCollection);
    ...
    private RequestsStatSummaryBo createSummaryData(Integer status, Collection<StartAndElapsedTime> timeCollection) {
        long totalTime = 0;
        long maxTime = 0;
        ...
        double avgTime = totalTime / timeCollection.size();

        RequestsStatSummaryBo agentRequestsStatSummaryData = new RequestsStatSummaryBo(url, status, timeCollection.size(), new Double(avgTime).longValue(), maxTime);
        agentRequestsStatSummaryData.setTimestamp(fastestEndTime);
        return agentRequestsStatSummaryData;
    }
