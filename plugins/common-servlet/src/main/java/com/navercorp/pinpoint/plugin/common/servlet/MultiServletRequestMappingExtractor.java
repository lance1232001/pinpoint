 ...
 * @author Taejin Koo
 */
public class MultiServletRequestMappingExtractor implements ServletRequestMappingExtractor {

    private List<ServletRequestMappingExtractor> servletRequestMappingExtractorList;

    public MultiServletRequestMappingExtractor(List<ServletRequestMappingExtractor> servletRequestMappingExtractorList) {
    ...
    @Override
    public String getUrl(HttpServletRequest target, String rawUrl) {

        for (ServletRequestMappingExtractor extractor : servletRequestMappingExtractorList) {
            String url = extractor.getUrl(target, null);
            if (url != null) {
                return url;
            }
        }
